...
from nemo.collections.nlp.models.language_modeling.stable_gpt_model import StableGPTModel
...
def gpt_model(model_cfg, trainer_cfg, precision):
    ...
    model = StableGPTModel(cfg=cfg, trainer=trainer)
...
class TestGPTModel:
    ...
    def test_constructor(self, gpt_model):
        assert isinstance(gpt_model, StableGPTModel)

        num_weights = gpt_model.num_weights
        assert num_weights == 13173504

    @pytest.mark.unit
    def test_tokenizer(self, gpt_model, test_text):

        assert HAS_TIKTOKEN, "`tiktoken` is not installed"
        print(gpt_model.tokenizer)
        assert isinstance(gpt_model.tokenizer, TikTokenTokenizer), "Wrong tokenizer"
        assert gpt_model.tokenizer.name == 'TikTokenTokenizer', "Wrong tokenizer name"
        assert gpt_model.tokenizer.vocab_size == 100277, "Wrong vocab size"

        ids = [gpt_model.tokenizer.text_to_ids(text) for text in test_text]

        true_ids = [
            [15339, 11, 1917],
            [35124, 5573, 323, 8254, 1667, 4227],
            [7927, 892, 374, 7347],
            [2746, 499, 743, 9021, 2579, 292, 38662, 1579]
        ]
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            # 32,  # Not supported with `flash-attn``
        ],
    )
    @pytest.mark.unit
    def test_forward(self, gpt_model, test_text):
        ...
        if gpt_model.cfg['precision'] == 32:
            print("precision: 32 is not supported, using bfloat16 instead")
            dtype = torch.bfloat16
            gpt_model.cfg['precision'] = 'bf16'
