...
def get_tokenizer_list() -> List[str]:
    ...
    return ["tiktoken", "sentencepiece", "char", "word"] + list(s)
...
def get_tokenizer(
    ...
    if tokenizer_name == 'sentencepiece':
        ...
        return TikTokenTokenizer(encoding_name=tokenizer_model)
...
def get_nmt_tokenizer(
    ...
    if library == 'yttm':
        ...
        logging.info(f'Getting `tiktoken` tokenizer for encoding: {model_name}') 
        return TikTokenTokenizer(encoding_name=model_name)
    else:
